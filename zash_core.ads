...
package Zash_Core is
   ...
   package Parser is
      ...
      -- Activim is the currently active stage of interpretation and expansion.
      -- _Raw means that, as-is, nothing has been interpreted or parsed yet. With
      -- this activim the parser can handle embedded strings or strings passed
      -- as arguments to some kind of evaluation command.
      -- _Control means that this is a word or symbol that will not be evaluated
      -- any further and will be used to control the execution flow.
      type Activim is mod 2 ** 64;
      Raw : constant Activim := 2 ** 0; -- utf8 string to be parsed as if from script, input or evaluate operation).
      History : constant Activim := 2 ** 1; -- :A -like Modifiers planned
      Control : constant Activim := 2 ** 2; -- Control structure meant for Zash parsing only.
      Word : constant Activim := 2 ** 4; -- Unit equal to an array element or argument, appropriate for alias or function checking.
      Aliasing : constant Activim := 2 ** 6; -- Match commands or arguments against aliasing.
      SH_Glob : constant Activim := 2 ** 7; -- A hack for SH compatibility.
      Process_FD : constant Activim := 2 ** 8; -- Placeholder for future use when emulating Zsh.
      Parameter : constant Activim := 2 ** 10; -- Designated for parameter expansion.
      Command : constant Activim := 2 ** 12; -- "Expect" output for Command.
      Arithmetic_X : constant Activim := 2 ** 14; -- Like Parameter, expand math.
      Brace : constant Activim := 2 ** 15; -- Brace Expansion.
      Glob : constant Activim := 2 ** 20; -- a Word that contains a glob pattern to expand or match.  always done last.

      type Item is private;
      function Copy (Self : in Item) return Item;
      function Value (Self : in Item) return Wide_String;
      function Stage (Self : in Item) return Activim;
      function Create (Source : Wide_String) return Item;
      function Create (Source : Wide_String; Stage : Activim) return Item;
      function "+" (Source : Wide_String) return Item;
      function "+" (Left : in Activim; Right : in Wide_String) return Item;
      type Item_Array is array (Positive) of Item;
   ...
   private
      ...
      overriding procedure Finalize (Object : in out Item);

      package Of_Item is new Ada.Containers.Indefinite_Vectors (Positive, Item);
      type Item_Vector is record
         Vector : Of_Item.Vector;
      end record;
